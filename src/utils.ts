...
export function renameJsxAttribute(
  ...
  return ts.factory.updateJsxAttribute(
    node,
    ts.factory.createIdentifier(newName),
    newInitializer ?? node.initializer,
  );
}
...
export function isInsideJsxElement(node: ts.Node, tagName: string): node is ts.Identifier {
  let el = node.parent;
  while (el && !isJsxElement(el, tagName)) {
    el = el.parent;
  }
  return !!el;
}
...
export function getParentJsxTag(
  node: ts.Node,
